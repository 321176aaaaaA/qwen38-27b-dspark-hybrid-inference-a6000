...
## 2026-08-22 更新：INT4 KV cache（生产已切换）

`--kv-cache-dtype int4_per_token_head`（vLLM 0.26 上游自带 RHT+非对称 zp 实现）+ 本仓库两个 patch（bf16 tensor-core dot、prefill 读放大修复、spec-decode int4 路由）：

- **KV 容量 1.73x**：220K 上下文并发 2.71x → 5.06x（~1.11M tok @ MEM_UTIL 0.95）
- **质量无损**：GPQA 32 题子集 93.8% 与 int8 同分同错题；NIAH 5 针 @190K 与 int8 同为 5/5
- **prefill 追平并反超 int8**（修掉了上游 BLOCK_Q=2 的深上下文读放大）；短上下文 decode 略慢 ~10%，长上下文更快
- 启动需 `--max-num-batched-tokens >= 3136`（mamba align）；生产脚本见 `scripts/start_vllm_official_200k_int4.sh`
- 详见 REPORT.md 末节
